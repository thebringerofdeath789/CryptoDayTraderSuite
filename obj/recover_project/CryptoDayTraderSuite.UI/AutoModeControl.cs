	...
	public partial class AutoModeControl : UserControl
	{
		...
		private int GetPersistedOpenCount(AccountInfo acc)
		{
			...
			try
			{
				...
				List<TradeRecord> exchangeTrades = trades.Where((TradeRecord t) => t != null && t.Executed && string.Equals(t.Exchange, acc.Service, StringComparison.OrdinalIgnoreCase)).ToList();
		...
		private void RefreshLatestTelemetrySummary()
		{
			...
			try
			{
				...
				lblTelemetrySummary.Text = string.Format("Telemetry: {0} | profiles {1}/{2} | executed {3} | blocked {4} | gates {5} (no-signal:{6}, ai-veto:{7}, risk-veto:{8}, success:{9}) | matrix {10} (guardrails:{11}, containment:{12}) | {13}", latest.Name, telemetry.ProcessedProfileCount, telemetry.EnabledProfileCount, telemetry.ExecutedProfiles, telemetry.FailedProfiles, string.IsNullOrWhiteSpace(telemetry.GateStatus) ? "n/a" : telemetry.GateStatus, telemetry.GateNoSignalObserved ? "obs" : "na", telemetry.GateAiVetoObserved ? "obs" : "na", telemetry.GateRiskVetoObserved ? "obs" : "na", telemetry.GateSuccessObserved ? "obs" : "na", string.IsNullOrWhiteSpace(telemetry.MatrixStatus) ? "n/a" : telemetry.MatrixStatus, telemetry.MatrixIndependentGuardrailsObserved ? "obs" : "na", telemetry.MatrixFailureContainmentObserved ? "obs" : "na", (telemetry.EndedUtc == default(DateTime)) ? "pending" : telemetry.EndedUtc.ToLocalTime().ToString("HH:mm:ss"));
		...
		private void EvaluateMatrixStatus(AutoCycleTelemetry cycleTelemetry)
		{
			...
			bool pass = cycleTelemetry.MatrixPairConfigurationConsistent && cycleTelemetry.MatrixIndependentGuardrailsObserved && cycleTelemetry.MatrixIsolationObserved;
			cycleTelemetry.MatrixStatus = (pass ? "PASS" : "PARTIAL");
		}
