...
Start-Process -FilePath $chromePath -ArgumentList @('--remote-debugging-port=9222', "--user-data-dir=$userData", '--disable-session-crashed-bubble', '--no-first-run', '--no-default-browser-check', 'https://chatgpt.com/') | Out-Null
...
$connected = $sidecar.ConnectAsync('ChatGPT').GetAwaiter().GetResult()
...
function Test-JsonSchemaCompliance {
    ...
    $startTag = 'CDTS_JSON_START'
    $endTag = 'CDTS_JSON_END'
    $startIdx = $clean.IndexOf($startTag)
    if ($startIdx -ge 0) {
        $fromStart = $startIdx + $startTag.Length
        $endIdx = $clean.IndexOf($endTag, $fromStart)
        if ($endIdx -gt $fromStart) {
            $clean = $clean.Substring($fromStart, $endIdx - $fromStart).Trim()
        }
        else {
            $clean = $clean.Substring($fromStart).Trim()
        }
    }
    $objStart = $clean.IndexOf('{')
    $objEnd = $clean.LastIndexOf('}')
    if ($objStart -ge 0 -and $objEnd -gt $objStart) {
        $clean = $clean.Substring($objStart, ($objEnd - $objStart + 1)).Trim()
    }
...
$connected2 = $sidecar2.ConnectAsync('ChatGPT').GetAwaiter().GetResult()
